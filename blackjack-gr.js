...
function displayTable() {	
	...
	if (!$.getElementById(cssId)){
	    ...
	    $.title = "Blackjack for Google Chrome!";
	}
	
	$.write (
		"<p id=\"game\">Game number: " + gameNumber + "</p>" 
		+ "<hr class=\"under2\" />"
		+ "<div id=\"bankroll\"></div>"
		+ "<div id=\"table\"><canvas id=\"cardTable\" width=\"600\" height=\"600\">You're just an analogue in a digital age, aren't ya.../n/nYour browser is too old and does not support the HTML5 canvas tag. Try playing <a href=\"blackjack.html\">blackjack.html</a> instead.</canvas></div>"
		+ "<div id='outcome'></div>"
		+ "<div id=\"buttons\">"
...
function clearText(coordsA,coordsB, color) {
	ctx.beginPath();
	ctx.arc(coordsA,coordsB,14,0,2*Math.PI);
...
function writeValueSplitCards(plyCardsValue,plyCardsValueB){
	if (split === 1) {	
		...
		clearText(165,360,"33ff33");
		clearText(165,460,"333333");
		setTextStyle();	

		if(plyCardsValue > 21) {
			ctx.fillStyle = "red";
			ctx.fillText(plyCardsValue,165,366);
		}
		else {
			ctx.fillText(plyCardsValue,165,366);
		}
		ctx.fillStyle = "white";
		ctx.fillText(plyCardsValueB,165,466);	
	}
	// Second Split Value
	else {
		clearText(165,360,"333333");
		clearText(165,460,"33ff33");
		setTextStyle();	
...
function writePlyValue(plyCardsValue) {
	clearText(165,429,"33ff33");
	setTextStyle();	

	if(plyCardsValue > 21) {
		ctx.fillStyle = "red";
		ctx.fillText(plyCardsValue,165,435);
	}
	else {
		ctx.fillText(plyCardsValue,165,435);
	}
...
function writeDeaValue(deaCardsValue) {
	clearText(165,159,"dd5500");
	setTextStyle();	
	ctx.fillText(deaCardsValue,165,165);
}
...
function drawDeaCard(deaCard) {
	...
	cardImg.style.zIndex = zeeIndex;
	cardImg.onload = setTimeout(function() {
		ctx.drawImage(cardImg,hposDea,125);
		hposDea += 25;
	}, delayDea);
	delayDea += 300;
	playSound("card");
...
function firstRound() {
	...
	plyImg01.onload = function() {
		ctx.drawImage(plyImg01,200,400);
	}
...
/* Commented out for debugging - this code displays the back of a card for the dealer's second (placeholder) card
	...
	deaImg02.onload = function() {
		ctx.drawImage(deaImg02,225,125);
	}
*/
...
function displaySplitCards(plyCard,deaCard,plyCard2) {
	...
	cardImg2.onload = setTimeout(function() {
		ctx.drawImage(cardImg2,200,125);
	}, 400);	
	...
	cardImg3.onload = setTimeout(function() {
		ctx.drawImage(cardImg3,200,425);
	}, 400);	
}
...
function blackJack() {		
	...
	standardBet = 1;
	oc.style.zIndex = 10;
	oc.innerHTML += ("<h3 class='win'>Congratulations, you have Blackjack!</h3>");
